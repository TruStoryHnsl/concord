...
describe("useServerConfigStore", () => {
  ...
  it("a non-transient pick persists as bootHome (not the in-session config) and round-trips via selectedHost()", () => {
    const cfg = sampleConfig();
    // A deliberate "this is my home" pick is non-transient.
    useServerConfigStore.getState().setHomeserver(cfg);

    // Only `bootHome` is persisted — the active `config` is in-session and
    // is restored FROM bootHome on launch (so a transient switch can't
    // become the boot home). See serverConfig.ts persist config.
    const raw = window.localStorage.getItem("concord_server_config");
    expect(raw).not.toBeNull();
    const parsed = JSON.parse(raw as string);
    expect(parsed.state.bootHome).toEqual(cfg);
    expect(parsed.state.config).toBeUndefined();

    // In-memory active state reflects the pick immediately.
    expect(useServerConfigStore.getState().config).toEqual(cfg);
    expect(useServerConfigStore.getState().bootHome).toEqual(cfg);
    expect(useServerConfigStore.getState().selectedHost()).toBe(
      "example.test",
    );
  });

  it("a transient switch updates config but NOT bootHome and is not persisted", () => {
    // Reset both slots first — clearHomeserver intentionally preserves
    // bootHome, so a prior test's home would otherwise leak in.
    useServerConfigStore.setState({ config: null, bootHome: null });
    window.localStorage.removeItem("concord_server_config");
    const cfg = sampleConfig();
    useServerConfigStore.getState().setHomeserver(cfg, { transient: true });
    // Active config reflects the switch in-session...
    expect(useServerConfigStore.getState().config).toEqual(cfg);
    // ...but bootHome stays null and nothing about the switch is persisted.
    expect(useServerConfigStore.getState().bootHome).toBeNull();
    const raw = window.localStorage.getItem("concord_server_config");
    const parsed = raw ? JSON.parse(raw) : { state: {} };
    expect(parsed.state.bootHome ?? null).toBeNull();
  });
