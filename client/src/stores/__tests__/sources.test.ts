...
describe("sources store", () => {
  ...
  it("hides (but keeps persisted) another user's sources when a different Concord user binds", () => {
    useSourcesStore.getState().bindToUser("@bob:example.concordchat.net");
    const state = useSourcesStore.getState();
    // NON-destructive: alice's row survives in the persisted set …
    expect(state.sources.map((source) => source.id)).toEqual([
      "src_matrix",
      "src_concord",
    ]);
    // … but bob's visible catalogue contains only the shared primary.
    expect(
      selectVisibleSources(state.sources, state.boundUserId).map((s) => s.id),
    ).toEqual(["src_concord"]);
  });
  ...
  it("does not transfer legacy unowned sources to a newly bound user", () => {
    ...
    const state = useSourcesStore.getState();
    // The legacy row is never re-attributed to the newly bound user
    // (an unowned row may carry someone else's session) …
    const legacy = state.sources.find((s) => s.id === "src_legacy");
    expect(legacy?.ownerUserId).toBeUndefined();
    // … and it is not in bo-peep's visible catalogue.
    expect(
      selectVisibleSources(state.sources, state.boundUserId).map((s) => s.id),
    ).toEqual(["src_concord"]);
  });
  ...
  it("shows only instance-global primary sources on logout", () => {
    useSourcesStore.getState().bindToUser(null);
    const state = useSourcesStore.getState();
    // Rows stay persisted (alice's catalogue survives her logout) …
    expect(state.sources.map((source) => source.id)).toEqual([
      "src_matrix",
      "src_concord",
    ]);
    // … but the logged-out view shows only the shared primary source.
    expect(
      selectVisibleSources(state.sources, state.boundUserId).map((s) => s.id),
    ).toEqual(["src_concord"]);
  });
