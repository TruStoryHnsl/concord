...
describe("FirstLaunchTwoNameBanner", () => {
  it("renders the welcome headline + guest-space sentence on a fresh install", () => {
    render(<FirstLaunchTwoNameBanner />);
    expect(
      screen.getByRole("heading", { name: /welcome.*name your space/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/guest space.*always-fresh entrance.*automatic/i),
    ).toBeInTheDocument();
  });
  ...
  it("renders both inputs with the documented placeholders", () => {
    ...
    expect(home.placeholder).toBe("My space");
  });
