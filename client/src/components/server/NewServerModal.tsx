  ...
  return (
    <form onSubmit={handleCreate} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-on-surface mb-1">
          Abbreviation
          <span className="text-on-surface-variant font-normal ml-1">(optional, 3 chars max)</span>
        </label>
        <input
          type="text"
          value={abbreviation}
          ...
          className="w-24 px-3 py-2 bg-surface border border-outline-variant rounded text-sm text-on-surface text-center placeholder-on-surface-variant/50 focus:outline-none focus:ring-1 focus:ring-primary/30"
        />
      </div>
      ...
      <button
        ...
        className="w-full py-2 primary-glow hover:brightness-110 disabled:opacity-40 text-on-surface font-medium rounded transition-colors"
      >
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="max-h-64 overflow-y-auto space-y-1">
        {loading ? (
          <p className="text-on-surface-variant text-sm text-center py-4">Searching...</p>
        ) : results.length === 0 ? (
          ...
          results.map((s) => (
            <div
              key={s.id}
              className="flex items-center justify-between px-3 py-2 rounded bg-surface-container-low hover:bg-surface-container-high transition-colors"
            >
              <div className="flex items-center gap-2 min-w-0">
                <div className="w-8 h-8 rounded-xl bg-surface-container-highest flex items-center justify-center text-xs font-bold text-on-surface-variant flex-shrink-0">
              ...
              <button
                ...
                className="text-xs px-3 py-1 primary-glow hover:brightness-110 disabled:opacity-40 text-on-surface rounded transition-colors flex-shrink-0"
              >
                {joining === s.id ? "..." : "Join"}
              </button>
            </div>
