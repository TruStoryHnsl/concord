...
// Phase 10 (bundle split): the <LiveKitRoom> tree lives in the lazy
// VoiceRoomLayer now, not App.tsx.
...
describe("[TESTER] FloatingVideoTiles — structural + dependency cross-checks", () => {
  describe("A. mount context", () => {
    it("mounts inside the VoiceRoomLayer LiveKitRoom (sibling of CustomAudioRenderer)", () => {
      ...
      const livekitRoomMatch = voiceRoomLayerSource.match(
        /<LiveKitRoom[\s\S]*?>([\s\S]*?)<\/LiveKitRoom>/,
      );
      expect(livekitRoomMatch, "VoiceRoomLayer must contain a <LiveKitRoom>...</LiveKitRoom>").not.toBeNull();
