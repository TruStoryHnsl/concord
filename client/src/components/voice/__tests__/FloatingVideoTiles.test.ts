...
// Phase 10 (bundle split): the live <LiveKitRoom> tree moved out of App.tsx
// into the lazy-loaded VoiceRoomLayer; FloatingVideoTiles is rendered there.
import voiceRoomLayerSource from "../VoiceRoomLayer.tsx?raw";
...
describe("FloatingVideoTiles — structural invariants for Issue E", () => {
  it("is imported and rendered inside LiveKitRoom in VoiceRoomLayer", () => {
    expect(voiceRoomLayerSource).toContain(
    ...
    expect(voiceRoomLayerSource).toMatch(/<CustomAudioRenderer \/>\s*(?:\{\/\*[\s\S]*?\*\/\}\s*)?<FloatingVideoTiles \/>/);
  });
