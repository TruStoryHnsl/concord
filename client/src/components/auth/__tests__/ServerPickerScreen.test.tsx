...
// Mock the Matrix probe (feat/federation-ui-w4): the matrix origin now
// verifies `/_matrix/client/versions` before claiming success. Tests
// control reachability through this mock instead of real network I/O.
vi.mock("../../../api/matrix", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../../../api/matrix")>();
  return {
    ...actual,
    probeMatrixHomeserver: vi.fn(),
  };
});
...
const mockedProbeMatrix = vi.mocked(matrixApi.probeMatrixHomeserver);
...
describe("<ServerPickerScreen />", () => {
  ...
  describe("desktop menu (Matrix / Host / Skip)", () => {
    ...
    it("Matrix card probes the homeserver and shows success for a reachable host", async () => {
      // feat/federation-ui-w4: the matrix origin no longer fabricates a
      // success screen. Well-known discovery failing is tolerated
      // (vanilla homeserver), but /_matrix/client/versions must answer.
      mockedDiscover.mockRejectedValue(new DnsResolutionError("matrix.org"));
      mockedProbeMatrix.mockResolvedValue(["v1.10"]);
      ...
      expect(mockedProbeMatrix).toHaveBeenCalledWith("https://matrix.org");
    ...
    it("Matrix card surfaces an error instead of success when the host is not a Matrix homeserver", async () => {
      mockedDiscover.mockRejectedValue(new DnsResolutionError("not-matrix.example"));
      mockedProbeMatrix.mockRejectedValue(
        new matrixApi.NotMatrixServerError(
          "https://not-matrix.example",
          "HTTP 404 from /_matrix/client/versions",
        ),
      );
      ...
      await user.click(screen.getByTestId("server-picker-choose-matrix"));
      const input = screen.getByTestId("server-picker-hostname-input");
      await user.type(input, "not-matrix.example");
      await user.click(screen.getByTestId("server-picker-connect-button"));

      await waitFor(() =>
        expect(screen.getByTestId("server-picker-error")).toBeInTheDocument(),
      );
      expect(screen.queryByTestId("server-picker-success")).not.toBeInTheDocument();
      expect(screen.getByTestId("server-picker-error")).toHaveTextContent(
        /doesn't look like a Matrix homeserver/i,
      );
    });
  });
});
