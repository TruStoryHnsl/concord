...
// The intrinsic local tile only renders when this device is itself hosting —
// `showLocalTile = isTauri() || knownPeers.length > 0` (added after these tests
// were written, which is why they regressed: in jsdom isTauri() is false and
// there are no peers, so nothing rendered). Mock it true so the tile renders.
