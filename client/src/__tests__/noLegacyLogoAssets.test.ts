 ...
 * favicon).
 *
 * The animated splash (`client/public/boot-splash.webp`) is the
 * user's hand-exported frame-by-frame render with a REAL alpha
 * channel (branding/BOOT-SPLASH.md) and is canonical. It is NOT
 * banned. The old opaque boot-splash.{webm,mp4} videos were fakes
 * (yuv420p on black) and are gone — do not resurrect them.
 *
 ...
 *      `"boot-splash.webp"` misuse). Comments are stripped before checking
...
const FORBIDDEN_FILES = [
  ...
  // The GIF iteration of the splash from the format experimentation
  // phase. The canonical animation is the hand-exported TRANSPARENT
  // animated WebP (branding/BOOT-SPLASH.md) — client/public/
  // boot-splash.webp is that asset and is NOT banned.
...
describe("regression guard: legacy logo assets are permanently banned", () => {
  ...
  it("the hand-crafted splash animation is present", () => {
    const splash = join(REPO_ROOT, "client/public/boot-splash.webp");
    if (!existsSync(splash)) {
      throw new Error(
        "boot-splash.webp is missing. This is the user's hand-exported " +
