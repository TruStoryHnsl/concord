 ...
 * interactive surface — NOT `ServerPickerScreen` (legacy pre-native
 * gate), NOT `LoginForm` (per-source sign-in lives inside the Add
 * Source flow), NOT `DockerFirstBootScreen` (web/Docker only).
 *
 * (The Welcome screen component was deleted as dead code — nothing
 * routes to it; App.tsx sends web builds to ServerPickerScreen. The
 * "no Welcome on native" contract is now enforced structurally by its
...
describe("first-launch (Windows native): __TAURI_INTERNALS__ + empty serverConfig", () => {
  ...
  it("takes the native-empty branch and renders ChatLayout as the first interactive surface", async () => {
    ...
    // install. ServerPickerScreen is the legacy pre-native gate;
    // LoginForm only renders on web post-picker; DockerFirstBootScreen
    // is web/Docker only.
