...
describe("peerStore API wrapper", () => {
  ...
  /**
   * Regression guard for the real wire form — source_to_wire in
   * src-tauri/src/lib.rs emits lowercase ("qr", "deeplink", "matrix_room",
   * "dht", "proximity") but the original parseSource only matched PascalCase.
   * This test locks in that lowercase variants (including "proximity") map
   * to the correct PeerSource value.
   */
  it("fetchKnownPeers parses lowercase wire source 'proximity' correctly", async () => {
    invokeMock.mockResolvedValueOnce([
      {
        peerId: "12D3KooWProximityPeer",
        publicKeyHex: "0".repeat(64),
        multiaddrs: ["/ip4/192.168.1.50/tcp/4001"],
        source: "proximity",
        firstSeen: "2026-06-05T10:00:00Z",
        lastSeen: "2026-06-05T10:01:00Z",
        accessGranted: true,
    ...
    const result = await fetchKnownPeers();
    expect(result).toHaveLength(1);
    expect(result[0].source).toBe("proximity");
    expect(result[0].peerId).toBe("12D3KooWProximityPeer");
  });
