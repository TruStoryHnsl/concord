...
const MAUTRIX_DISCORD_ASSET: &str = "mautrix-discord-amd64";

/// Port for the provisioning API (guild listing, bridging).
/// Distinct from the appservice port so both can coexist.
const _PROVISIONING_PORT: u16 = 29_335;
...
async fn install_bwrap() -> Result<(), BridgeCommandError> {
    Err(BridgeCommandError::Transport(
        crate::servitude::transport::TransportError::NotImplemented(
            "bubblewrap auto-install is Linux-only",
        ),
    ))
}
...
async fn set_bot_token_inner(token: String) -> Result<(), BridgeCommandError> {
    ...
    // Generate a provisioning shared secret for the guild management API.
    let prov_secret = generate_hex_token();

    // SQLite database path — uses /data/ because the bwrap sandbox
    // mounts the data directory there, not at the host path.
    let db_uri = "file:/data/mautrix-discord.db?_txlock=immediate".to_string();

    // Write a complete config.yaml with the real bot token.
    let config_contents = format!(
        "# Generated by Concord (INS-024)\n\
         homeserver:\n\
         \x20\x20address: http://127.0.0.1:{listen_port}\n\
         \x20\x20domain: {server_name}\n\
         \x20\x20software: standard\n\
         appservice:\n\
         ...
         \x20\x20bot:\n\
         \x20\x20\x20\x20username: _discord_bot\n\
         \x20\x20\x20\x20displayname: Discord Bridge Bot\n\
         \x20\x20database:\n\
         \x20\x20\x20\x20type: sqlite3-fk-wal\n\
         \x20\x20\x20\x20uri: {db_uri}\n\
         bridge:\n\
         \x20\x20username_template: _discord_{{{{.}}}}\n\
         \x20\x20displayname_template: '{{{{or .GlobalName .Username}}}} (Discord)'\n\
         \x20\x20command_prefix: '!discord'\n\
         \x20\x20portal_message_buffer: 128\n\
         \x20\x20permissions:\n\
         \x20\x20\x20\x20'*': relay\n\
         \x20\x20\x20\x20'{server_name}': admin\n\
         discord:\n\
         \x20\x20token: {bot_token}\n\
         provisioning:\n\
         \x20\x20prefix: /_matrix/provision\n\
         \x20\x20shared_secret: {prov_secret}\n\
         logging:\n\
         \x20\x20min_level: info\n\
         \x20\x20writers:\n\
         \x20\x20- type: stdout\n\
         \x20\x20\x20\x20format: pretty-colored\n",
...
async fn check_bot_token_exists() -> bool {
    ...
    match tokio::fs::read_to_string(&config_path).await {
        Ok(contents) => {
            // Check both old field name (bot_token) and new (token under discord:).
            let token = extract_yaml_value(&contents, "token")
                .or_else(|| extract_yaml_value(&contents, "bot_token"));
            match token {
                Some(t) => !t.is_empty() && !t.contains("PLACEHOLDER"),
                None => false,
            }
        }
        Err(_) => false,
...
async fn ensure_binary_inner() -> Result<String, BridgeCommandError> {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        return Err(BridgeCommandError::Transport(
            crate::servitude::transport::TransportError::NotImplemented(
                "Discord bridge auto-download is Linux-only",
            ),
        ));
    }
...
// -----------------------------------------------------------------------
// Provisioning API proxies — guild listing, bridging, unbridging
// -----------------------------------------------------------------------

/// Read the provisioning shared secret from the bridge config.yaml.
async fn read_provisioning_secret() -> Result<String, BridgeCommandError> {
    let data_dir = DiscordBridgeTransport::resolve_data_dir()
        .map_err(|e| BridgeCommandError::DataDirResolution(e.to_string()))?;
    let config_path = data_dir.join("config.yaml");
    let contents = tokio::fs::read_to_string(&config_path)
        .await
        .map_err(|e| BridgeCommandError::ConfigWrite(format!("failed to read config: {}", e)))?;
    extract_yaml_value(&contents, "shared_secret")
        .ok_or_else(|| BridgeCommandError::ConfigWrite(
            "provisioning shared_secret not found in config.yaml".to_string(),
        ))
}

/// Discord guild as returned by the provisioning API.
...
#[tauri::command]
pub async fn discord_bridge_list_guilds() -> Result<Vec<DiscordGuild>, String> {
    list_guilds_inner().await.map_err(|e| e.to_string())
}

async fn list_guilds_inner() -> Result<Vec<DiscordGuild>, BridgeCommandError> {
    let secret = read_provisioning_secret().await?;
    let port: u16 = crate::servitude::transport::discord_bridge::DEFAULT_BRIDGE_PORT;
    let url = format!(
        "http://127.0.0.1:{}/_matrix/provision/v1/guilds",
        port
    );
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| BridgeCommandError::Transport(
            crate::servitude::transport::TransportError::StartFailed(
                format!("provisioning API unreachable: {}", e),
            ),
        ))?;
    ...
    if !resp.status().is_success() {
        ...
        return Err(BridgeCommandError::Transport(
            crate::servitude::transport::TransportError::StartFailed(
                format!("guild list failed (HTTP {}): {}", status, redact_for_logging(&body)),
            ),
        ));
    }

    let guilds: Vec<DiscordGuild> = resp.json().await.map_err(|e| {
        BridgeCommandError::Transport(
            crate::servitude::transport::TransportError::StartFailed(
                format!("failed to parse guild list: {}", e),
            ),
        )
    })?;
...
/// Bridge a specific Discord guild by ID.
#[tauri::command]
pub async fn discord_bridge_guild(guild_id: String) -> Result<(), String> {
    bridge_guild_inner(guild_id).await.map_err(|e| e.to_string())
}

async fn bridge_guild_inner(guild_id: String) -> Result<(), BridgeCommandError> {
    let secret = read_provisioning_secret().await?;
    let port: u16 = crate::servitude::transport::discord_bridge::DEFAULT_BRIDGE_PORT;
    let url = format!(
        "http://127.0.0.1:{}/_matrix/provision/v1/guilds/{}",
    ...
    if !resp.status().is_success() {
        ...
        return Err(BridgeCommandError::Transport(
            crate::servitude::transport::TransportError::StartFailed(
                format!("bridge guild failed (HTTP {}): {}", status, redact_for_logging(&body)),
            ),
        ));
    }
...
/// Unbridge a Discord guild by ID.
#[tauri::command]
pub async fn discord_bridge_unbridge_guild(guild_id: String) -> Result<(), String> {
    unbridge_guild_inner(guild_id).await.map_err(|e| e.to_string())
}

async fn unbridge_guild_inner(guild_id: String) -> Result<(), BridgeCommandError> {
    let secret = read_provisioning_secret().await?;
    let port: u16 = crate::servitude::transport::discord_bridge::DEFAULT_BRIDGE_PORT;
    let url = format!(
        "http://127.0.0.1:{}/_matrix/provision/v1/guilds/{}",
    ...
    if !resp.status().is_success() {
        ...
        return Err(BridgeCommandError::Transport(
            crate::servitude::transport::TransportError::StartFailed(
                format!("unbridge guild failed (HTTP {}): {}", status, redact_for_logging(&body)),
            ),
        ));
    }
